...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const embed = new EmbedBuilder().setColor(color).setTitle("Select New Rank")
  ...
  try {
    ...
    if (guild[0]?.logChannelId) {
      ...
      if (ch?.isTextBased()) {
        await (ch as any).send({ embeds: [new EmbedBuilder().setColor(color).setTitle("Staff Promoted").addFields({ name: "Staff", value: `<@${target.id}>`, inline: true }, { name: "By", value: `<@${interaction.user.id}>`, inline: true }, { name: "From", value: currentRank?.name ?? "None", inline: true }, { name: "To", value: selectedRank.name, inline: true }, { name: "Reason", value: reason }).setFooter({ text: footer }).setTimestamp()] });
      }
    }
  } catch {
