...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (allStaff.length === 0) {
    const embed = new EmbedBuilder().setColor(color).setTitle("Staff Roster")
  ...
  const embed = new EmbedBuilder().setColor(color).setTitle(`Staff Roster — ${interaction.guild.name}`)
    .setDescription(
      pageData.map((s, i) => {
