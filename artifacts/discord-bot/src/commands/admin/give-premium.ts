...
export const data = new SlashCommandBuilder()
  ...
  .setDescription("Grant premium access to a server")
  ...
  try {
    ...
    await dm.send({
      embeds: [successEmbed(
        ...
        `Your server **${guild[0].name}** now has access to Zenith Premium features!\n${expiryText}\n\nVisit the dashboard to unlock all premium features.`,
        "#FFD700",
        "Zenith Staff Management",
      )],
    });
  } catch { /* DMs disabled or guild not accessible */ }
}
