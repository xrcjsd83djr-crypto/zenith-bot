...
import { staffTable, strikesTable, guildsTable } from "@workspace/db/schema";
...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((opt) =>
    opt.setName("reason").setDescription("Reason for the strike").setRequired(true)
  )
  .addStringOption((opt) =>
    opt
      .setName("severity")
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const [strikeCountResult] = await db
    .select({ total: count() })
    .from(strikesTable)
  ...
  try {
    ...
    const dmEmbed = new EmbedBuilder()
      .setColor("#ED4245")
      .setTitle(`You have received a ${severityLabel} in ${interaction.guild.name}`)
      .addFields(
        { name: "Reason", value: reason },
        { name: "Issued By", value: interaction.user.username },
        ...(evidence ? [{ name: "Evidence", value: evidence }] : []),
        { name: "Total Active Strikes", value: `${newCount}` },
      )
  ...
  if (guild[0]?.logChannelId) {
    ...
    if (logChannel?.isTextBased()) {
      const logEmbed = new EmbedBuilder()
        .setColor("#ED4245")
        .setTitle(`Strike Issued — ${severityLabel}`)
        .addFields(
          { name: "Staff Member", value: `<@${target.id}>`, inline: true },
          { name: "Issued By", value: `<@${interaction.user.id}>`, inline: true },
          { name: "Active Strikes", value: `${newCount}`, inline: true },
          { name: "Reason", value: reason },
          ...(evidence ? [{ name: "Evidence", value: evidence }] : []),
        )
        .setFooter({ text: footer })
