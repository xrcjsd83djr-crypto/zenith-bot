...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const activeStrikes = await db.select().from(strikesTable)
    .where(and(eq(strikesTable.staffId, staffMember[0].id), eq(strikesTable.isActive, true))).limit(1);
  ...
  await interaction.editReply({
    embeds: [successEmbed(
      "Strike Removed",
      `Removed a strike from **${target.username}**${reason ? `\n**Reason:** ${reason}` : ""}\nThey now have **${remaining.total}** active strike${remaining.total !== 1 ? "s" : ""}.`,
      color, footer,
    )],
  });
}
