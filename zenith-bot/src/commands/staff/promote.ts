...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (availableRanks.length === 0) {
    await interaction.editReply({ embeds: [errorEmbed("No other ranks to promote to.")] });
    return;
  }

  const select = new StringSelectMenuBuilder().setCustomId("promote_rank").setPlaceholder("Select new rank")
    .addOptions(availableRanks.slice(0, 25).map((r) => ({ label: r.name, value: r.id })));

  const embed = new EmbedBuilder().setColor(color as any).setTitle("Select New Rank")
  ...
  try {
    ...
    await db.insert(promotionsTable).values({ id: randomUUID().replace(/-/g,"").slice(0,21), guildId: interaction.guild.id, staffId: staffMember[0].id, type: "promotion", fromRankId: currentRank?.id ?? null, toRankId: selectedRank.id, fromRankName: currentRank?.name ?? null, toRankName: selectedRank.name, reason, promotedById: interaction.user.id, promotedByUsername: interaction.user.username });
    ...
    await sel.update({ embeds: [successEmbed("Staff Promoted", `**${target.username}** → **${selectedRank.name}**\n**Reason:** ${reason}`, color, footer)], components: [] });

    // Notify the staff member via DM
    try {
      const dm = await target.createDM();
      await dm.send({ embeds: [successEmbed("Congratulations!", `You have been promoted to **${selectedRank.name}** in **${interaction.guild.name}**!\n**Reason:** ${reason}`, color, footer)] });
    } catch { /* DMs disabled */ }
    ...
    if (guild[0]?.logChannelId) {
      ...
      if (ch?.isTextBased()) {
        await (ch as any).send({ embeds: [new EmbedBuilder().setColor(color as any).setTitle("Staff Promoted").addFields({ name: "Staff", value: `<@${target.id}>`, inline: true }, { name: "By", value: `<@${interaction.user.id}>`, inline: true }, { name: "From", value: currentRank?.name ?? "None", inline: true }, { name: "To", value: selectedRank.name, inline: true }, { name: "Reason", value: reason }).setFooter({ text: footer }).setTimestamp()] });
      }
    }
  } catch {
